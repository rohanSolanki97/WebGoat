...
public class SqlInjectionLesson8 implements AssignmentEndpoint {
  ...
  protected AttackResult injectableQueryConfidentiality(String name, String auth_tan) {
    ...
    String query = "SELECT * FROM employees WHERE last_name = ? AND auth_tan = ?"; // '?' placeholders: first for name, second for auth_tan
    ...
    try (Connection connection = dataSource.getConnection()) {
      try {
        ...
        ps.setString(1, name);
        ps.setString(2, auth_tan); // Bind the user input parameters
        // Optionally log the prepared statement and parameters if needed
